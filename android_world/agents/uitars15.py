...
class UITARS15Agent(base_agent.EnvironmentInteractingAgent):
    ...
    def step(self, goal: str) -> base_agent.AgentInteractionResult:
        """Perform a step of the agent on the environment."""
        step_data = {
            "raw_screenshot": None,
            "screen_size": None,
            "raw_response": None,
        ...
        # Prepare observation for UITARS15
        obs = {
        ...
        try:
            # Use UITARS15 prediction method
            prediction, actions = self._predict_uitars15(goal, obs)
            ...
            # Process the first action (UITARS15 can return multiple actions)
            if actions and len(actions) > 0:
                # Parse the action from UITARS15 format
                ...
                if parsed_actions:
                    ...
                    # Convert UITARS15 action to android_world action
                    result_action = uitars15_action_to_android_action(
                    ...
                    # Handle attack detection (same as other agents)
                    if (
            ...
            logger.error(f"Error in UITARS15 step: {e}")
