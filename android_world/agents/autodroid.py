...
ACTION_SELECTION_PROMPT_TEMPLATE = (
    ...
    ' 4. Can the task be proceeded with the current UI state? Out put an action.'
...
def parse_reason_action_output(
  ...
  reason_result = re.search(
      r'Next step:(.*)Action:', raw_reason_action_output, flags=re.DOTALL
  )
...
Action: {{"action_type": "status", "goal_status": "infeasible"}}"""
    ...
    next_step,action = parse_reason_action_output(action_output)

    # If the output is not in the right format, add it to step summary which
    # will be passed to next step and return.
    if (not next_step or not action):
